...
public class WarehouseProductService
{
    ...
    public async Task<BulkStockCheckResultDto> ValidateBulkStockAsync(BulkStockCheckDto dto)
    {
        ...
        {
            var totalStock = await _repository.GetTotalStockByProductAsync(item.ProductId);
            var product = await _repository.GetProductNameAsync(item.ProductId);
            var required = item.Quantity;
            lines.Add(new StockCheckLineDto
            {
                ProductId = item.ProductId,
                ProductName = product ?? $"Product {item.ProductId}",
                Required = required,
        ...
        return new BulkStockCheckResultDto
        {
            AllAvailable = lines.All(l => l.Sufficient),
            Lines = lines
        };
    }
    ...
    public async Task<BulkStockDeductResultDto> DeductBulkStockAsync(BulkStockDeductDto dto)
    {
        foreach (var item in dto.Items)
        {
            var qty = (int)Math.Ceiling(item.Quantity);
            var warehouseId = await _repository.GetFirstWarehouseWithStockAsync(item.ProductId, qty);
            if (!warehouseId.HasValue)
            {
                return new BulkStockDeductResultDto
                {
                    Success = false,
            ...
            var result = await AdjustStockAsync(new StockChangeDto
            {
                ProductId = item.ProductId,
                WarehouseId = warehouseId.Value,
                Quantity = qty,
                ActionType = "EXIT",
                Reason = string.IsNullOrWhiteSpace(dto.Reason) ? "Sale" : dto.Reason
            });
        ...
        return new BulkStockDeductResultDto { Success = true, Message = "Stock deducted successfully." };
    }
